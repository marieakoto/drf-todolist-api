...
class TestTodoDetailAPIView(TodosAPITestCase):
    ...
    def test_updates_one_item(self):
        ...
        res = self.client.patch(reverse('todos-detail', kwargs = {'id': response.data['id']}), {"title": "New one", 'is_completed': True})
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_deletes_one_item(self):
        ...
        res = self.client.delete(reverse('todos-detail', kwargs = {'id': response.data['id']}))
