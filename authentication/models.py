...
from authentication.exceptions import UserValidator
...
class MyUserManager(UserManager):         ##This manager is going to provide methods for creating regular users and superusers.Allows to customize how queries are performed on the model.
    ...
    def create_user(self, username, email, password=None, **extra_fields):        
        ...
        UserValidator.validate_username(username)
        UserValidator.validate_email(email)
        UserValidator.validate_user_fields(extra_fields)
    ...
    def create_superuser(self, username, email, password=None, **extra_fields):         
        ...
        UserValidator.validate_username(username)
        UserValidator.validate_email(email)
        UserValidator.validate_superuser_fields(extra_fields)
...
class User(AbstractBaseUser, PermissionsMixin,TrackingModel): 
    ...
    username = models.CharField(_("username"), max_length=128, unique =True, validators=[username_validator],)
